...
void signalHandler(int signal) {
    ...
    // Non-blocking wait to clean up child processes
    while ((pid = waitpid(-1, NULL, WNOHANG)) > 0) {
        for (int i = 0; i < MAX_JOBS; ++i) {
            if (jobs[i].pid == pid) {
                char msg[256];
                snprintf(msg, sizeof(msg), "[%d] (%d)  finished  %s\n", jobs[i].jobID, pid, jobs[i].command);
                write(STDOUT_FILENO, msg, strlen(msg));

                // Clear the job entry by setting its jobID to 0
                jobs[i].jobID = 0;
                break;
            }
        }
        ...
        // If we're not the child and BG is true, we can keep running!
            ...
            for (int i = 0; i < MAX_JOBS; i++) {
                if (jobs[i].jobID == 0) {
                    ...
                    jobs[i].pid = p1;
                    jobs[i].command = toks[0];


                    char msg[100];
                    snprintf(msg, sizeof(msg), "[%d] (%d)  running  %s\n", jobIDCounter, p1, toks[0]);
                    jobIDCounter++;
                    write(STDOUT_FILENO, msg, strlen(msg));
                    break;
                }
